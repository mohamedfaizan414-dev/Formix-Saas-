...
export function alignmentClass(align) {
  switch (align) {
    case "center":
...
function getTypographyClasses(node) {
  ...
  const colorMap = { default: "text-ink dark:text-white", teal: "text-clinical-teal", sage: "text-clinical-sage", brick: "text-clinical-brick", soft: "text-ink-soft" };
  const familyMap = { sans: "font-sans", serif: "font-serif", mono: "font-mono", display: "font-display" };

  return cn(sizeMap[size], weightMap[weight], colorMap[color], familyMap[family]);
}

export function FieldShell({ node, error, children }) {
  ...
  return (
    <div className={cn("space-y-1 w-full box-border flex-none", alignmentClass(node.display?.align), accentClass(node.display?.colorAccent))}>
      <div className="w-full flex flex-col">
        {node.label && !isActionType && node.type !== "inlineTemplate" && (
...
function ReadOnlyComposite({ node, value }) {
...
function ReadOnlyValue({ node, value }) {
...
function VitalsFields({ value, onChange, disabled }) {
  const v = value || {};
...
function RepeatingRows({ value, onChange, disabled, columns }) {
...
function SignaturePad({ disabled, value, onChange }) {
  ...
  const start = (e) => { if (disabled) return; drawing.current = true; const ctx = canvasRef.current?.getContext("2d"); const rect = canvasRef.current.getBoundingClientRect(); ctx?.beginPath(); ctx?.moveTo(e.clientX - rect.left, e.clientY - rect.top); };
  const move = (e) => { if (!drawing.current || disabled) return; const ctx = canvasRef.current?.getContext("2d"); const rect = canvasRef.current.getBoundingClientRect(); if (ctx) { ctx.strokeStyle = "#132A33"; ctx.lineWidth = 2; ctx.lineTo(e.clientX - rect.left, e.clientY - rect.top); ctx.stroke(); } };
...
export function FieldRenderer({ node, value, onChange, disabled, interactive = true, readOnlyView = false, error, assignmentToken }) {
        ...
        <FieldShell node={node} error={error}>
          {(() => {
            switch (node.type) {
              ...
              case "htmlBlock": return <div className="prose prose-sm max-w-none break-words overflow-x-auto" dangerouslySetInnerHTML={{ __html: node.meta?.html ?? "" }} />;
              ...
              case "spacer": return <div style={{ height: Math.min(node.meta?.heightPx ?? 24, 48) }} />;
              default: return null;
            }
          })()}
        </FieldShell>
  ...
  const body = (() => {
    switch (node.type) {
      ...
      case "text": case "email": case "password": case "phone": case "hidden":
        return <Input {...commonProps} type={node.type === "hidden" ? "text" : node.type} value={value ?? ""} onChange={(e) => onChange(e.target.value)} className={node.type === "hidden" ? "hidden" : "w-full text-xs sm:text-sm"} />;
      case "number": return <Input {...commonProps} type="number" value={value ?? ""} onChange={(e) => onChange(e.target.value)} className="w-full text-xs sm:text-sm" />;
      case "textarea": return <Textarea {...commonProps} rows={node.meta?.rows ?? 4} value={value ?? ""} onChange={(e) => onChange(e.target.value)} className="w-full text-xs sm:text-sm" />;
      case "date": return <Input {...commonProps} type="date" value={value ?? ""} onChange={(e) => onChange(e.target.value)} className="w-full text-xs sm:text-sm min-h-[40px]" />;
      case "time": return <Input {...commonProps} type="time" value={value ?? ""} onChange={(e) => onChange(e.target.value)} className="w-full text-xs sm:text-sm min-h-[40px]" />;
      case "datetime": return <Input {...commonProps} type="datetime-local" value={value ?? ""} onChange={(e) => onChange(e.target.value)} className="w-full text-xs sm:text-sm min-h-[40px]" />;
      ...
      case "dropdown":
        return (
          <Select disabled={isDisabled} value={value ?? ""} onChange={(e) => onChange(e.target.value)} className="w-full text-xs sm:text-sm">
      ...
      case "rating": {
        const max = node.meta?.max ?? 5; const current = Number(value) || 0;
        return (
      ...
      case "slider": {
        const min = node.meta?.min ?? 0; const max = node.meta?.max ?? 100;
        return (
      ...
      case "file": case "image": case "medicalImage": {
        if (isDisabled) {
          if (value) {
            const isImage = String(value).match(/\.(jpeg|jpg|gif|png|webp|svg)/i) || node.type === "image" || node.type === "medicalImage";
            return (
              <div className="flex flex-col items-center gap-3 rounded-md border border-ink/10 p-4 bg-paper-dim dark:bg-white/5 w-full">
                {isImage ? (
                  <img src={value} alt="Uploaded attachment" className="max-h-60 max-w-full rounded-xs object-contain border border-ink/5 shadow-sm" />
                ) : (
                  <div className="text-xs text-ink-soft font-mono truncate max-w-full">
                ...
                <div className="flex items-center gap-2.5">
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="inline-flex items-center gap-1 text-xs text-clinical-teal hover:underline font-semibold"
                  >
                    Open full view
                  </a>
            ...
            return (
              <div className="rounded-xs border border-dashed border-ink/15 py-4 text-center text-xs text-ink-soft/50 w-full italic bg-ink/[0.01]">
                No file or image attached
              </div>
            );
          }
        }
            ...
            try {
              ...
              const res = await fetch("/api/upload", {
                ...
                headers: { 
                  ...
                  ...(assignmentToken ? { "X-Assignment-Token": assignmentToken } : {})
                },
                body: JSON.stringify({
                  dataUri,
        ...
        return (
          <label className={cn("flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xs border border-dashed border-ink/25 py-5 sm:py-6 px-4 text-xs sm:text-sm text-ink-soft transition-colors hover:border-clinical-sage text-center w-full min-h-[64px]", isDisabled && "pointer-events-none opacity-50 bg-ink/5")}>
            ...
            <input type="file" className="hidden" accept={node.meta?.accept} disabled={isDisabled} onChange={handleFile} />
          </label>
        );
      }
      ...
      case "htmlBlock": return <div className="prose prose-sm max-w-none break-words w-full overflow-x-auto" dangerouslySetInnerHTML={{ __html: node.meta?.html ?? "" }} />;
      ...
      case "spacer": return <div style={{ height: Math.min(node.meta?.heightPx ?? 24, 48) }} className="w-full" />;
      ...
      case "nursingNotes": return <Textarea {...commonProps} rows={5} value={value ?? ""} onChange={(e) => onChange(e.target.value)} placeholder="Progress notes…" className="w-full text-xs sm:text-sm" />;
      case "consent":
        return (
      ...
      default: return <Input {...commonProps} value={value ?? ""} onChange={(e) => onChange(e.target.value)} className="w-full text-xs sm:text-sm" />;
    }
  })();
