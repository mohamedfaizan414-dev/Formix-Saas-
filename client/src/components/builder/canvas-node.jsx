...
import { rectToGridStyle, GRID_COLUMNS, SPAN_TO_WIDTH, SNAP_SPANS } from "../../lib/form-engine/grid-engine";
import { useFlipAnimation } from "../../lib/form-engine/use-flip-animation";
import { cn } from "../../lib/utils/cn";

function nearestSpan(target) {
...
export function CanvasNode({
  ...
  useFlipAnimation(wrapperRef, `${rect.x}-${rect.y}-${rect.w}-${rect.h}`);
  ...
  const [resizePreviewSpan, setResizePreviewSpan] = React.useState(null);
  const resizeStart = React.useRef(null);

  function handleResizePointerDown(e) {
    e.stopPropagation();
    e.preventDefault();
    const container = wrapperRef.current?.closest("[data-grid-columns]");
            ...
            {["heading", "paragraph"].includes(node.type) ? (
              <div 
                className={cn(
                  ...
                  "list-[disc] list-inside ml-4 my-2",
