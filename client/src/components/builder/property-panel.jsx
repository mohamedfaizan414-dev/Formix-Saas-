...
export function PropertyPanel() {
  ...
  const patch = (p) => updateComponent(node.id, p);
  const patchValidation = (p) => patch({ validation: { ...node.validation, ...p } });
  const patchDisplay = (p) => patch({ display: { ...node.display, ...p } });
  const patchMeta = (p) => patch({ meta: { ...node.meta, ...p } });
            ...
            <div className="space-y-4">
              ...
              {supports.orientation && (
                <div className="space-y-1.5">
                  ...
                  <Select value={node.orientation ?? "vertical"} onChange={(e) => patch({ orientation: e.target.value })}>
              ...
              {supports.rows && (
                <div className="space-y-1.5">
                  <Label>Rows</Label>
                  <Input type="number" value={node.meta?.rows ?? 4} onChange={(e) => patch({ meta: { ...node.meta, rows: Number(e.target.value) } })} />
                </div>
              )}
              ...
              {node.type === "inlineTemplate" && (
                <InlineTemplatePanel
                  node={node}
                  onUpdate={(config) => patch({ config })}
                />
              )}
            </div>
          </TabsContent>
          ...
          <TabsContent value="display">
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Select value={node.display?.width ?? "full"} onChange={(e) => patchDisplay({ width: e.target.value })}>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={node.display?.align ?? "left"} onChange={(e) => patchDisplay({ align: e.target.value })}>
              ...
              {isTypography && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <Select value={node.meta?.fontSize ?? "base"} onChange={(e) => patchMeta({ fontSize: e.target.value })}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={node.meta?.fontWeight ?? "normal"} onChange={(e) => patchMeta({ fontWeight: e.target.value })}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={node.meta?.fontFamily ?? "sans"} onChange={(e) => patchMeta({ fontFamily: e.target.value })}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={node.meta?.textColor ?? "default"} onChange={(e) => patchMeta({ textColor: e.target.value })}>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={node.display?.colorAccent ?? "none"} onChange={(e) => patchDisplay({ colorAccent: e.target.value })}>
