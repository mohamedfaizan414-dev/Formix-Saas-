...
export function Switch({
  ...
  disabled?: boolean; // 🧠 Added type definition
  className?: string;
}) {
  return (
    <button
      ...
      disabled={disabled} // 🧠 Wired up the underlying HTML disabled state
      onClick={() => !disabled && onCheckedChange(!checked)} // 🧠 Added safety block
      className={cn(
        ...
        disabled && "opacity-40 cursor-not-allowed pointer-events-none", // 🧠 Dynamic styles for disabled look
        className
      )}
    >
      <span
