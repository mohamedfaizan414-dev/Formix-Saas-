...
// 🌟 IMPORT THESE EXTENSIONS EXPLICITLY TO ENABLE HEADINGS & LISTS
...
export function RichTextInput({ value, onChange }: RichTextInputProps) {
  const editor = useEditor({
    // 🌟 ADD THE EXTENSIONS TO THE ARRAY HERE SO TIPTAP PROCESSES H1/H2/UL/LI TAGS
    extensions: [
      StarterKit.configure({
        heading: false,     // Disable standard starter-kit headings to avoid configuration conflicts
        bulletList: false,  // Disable starter-kit bullet lists to use explicit definitions
      }),
      Underline,
      Heading.configure({
        levels: [1, 2],
...
React.useEffect(() => {
  if (editor && value !== editor.getHTML()) {
    // 🌟 FIXED: Change the raw boolean parameter to a proper options object literal
  ...
  return (
    <div className="rounded-md border border-ink/15 bg-white dark:bg-paper-darkdim overflow-hidden focus-within:border-clinical-teal transition-all w-full">
      {/* Toolbar row with flex-wrap fixes */}
      ...
      {/* Editor Content Area with List Styling Defaults */}
  <EditorContent 
  editor={editor} 
  className={cn(
    ...
    "list-[disc] list-inside ml-4 prose-li:my-0.5" // 👈 UNHIDES BULLETS IN ACTIVE TYPING VIEWS
  )} 
/>
    </div>
  );
