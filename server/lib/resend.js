...
const FROM = process.env.SMTP_FROM || process.env.RESEND_FROM_EMAIL || "Formix Clinical <onboarding@resend.dev>";
...
async function sendFormAssignmentEmail({
  to,
  patientName,
  formTitle,
  formDescription,
  ...
  if (!transporter) {
    console.warn("SMTP_HOST not set — skipping email send (dev mode). Link:", link);
    return;
  }
