...
export function IssueFormSection({ patientId, availableForms }: IssueFormSectionProps) {
  ...
  // 🌟 CHANGED: State is now an array of selected form IDs
  ...
  // Toggle form selection in array
  ...
  // Remove a pill token tag directly
  function removeForm(id: string, e: React.MouseEvent) {
    e.stopPropagation();
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      // 🌟 CHANGED: Sending an array of formIds instead of a single string
  ...
  return (
    <div className="space-y-3 rounded-md border border-ink/10 bg-white p-4 dark:border-white/10 dark:bg-paper-darkdim shadow-panel">
      ...
      <div className="relative w-full" ref={containerRef}>
        {/* Clickable multi-select panel base */}
        <div
        ...
        >
          {selectedFormIds.length === 0 ? (
            ...
            /* Selected Form Token Pills Matrix */
