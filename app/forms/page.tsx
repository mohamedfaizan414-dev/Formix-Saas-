    ...
    <AppShell active="forms">
      <div className="mx-auto max-w-5xl px-4 sm:px-8 py-6 sm:py-10">
        ...
        <div className="hidden sm:block overflow-hidden rounded-md border border-ink/10 dark:border-white/10 bg-white dark:bg-paper-darkdim">
          {/* 🌟 FIXED: Kept table layout strictly 'table-fixed' globally to maintain column dimensions */}
          <table className="w-full text-sm table-fixed">
            <thead className="bg-paper-dim text-left text-xs uppercase tracking-wide text-ink-soft dark:bg-white/5 dark:text-white/40">
              <tr>
                {/* 🌟 FIXED: Optimized widths to prevent text blocks from expanding into actions spacing */}
                <th className="px-4 py-3 font-medium w-1/3">Name</th>
                {session.role === "SUPER_ADMIN" && <th className="px-4 py-3 font-medium w-1/4">Hospital</th>}
                <th className="px-4 py-3 font-medium w-24">Status</th>
                <th className="px-4 py-3 font-medium w-20">Version</th>
                <th className="px-4 py-3 font-medium w-28">Updated</th>
                {/* 🌟 FIXED: Locked width to guarantee visibility on ultra-wide screens */}
                <th className="px-4 py-3 font-medium text-right w-56" />
              </tr>
            </thead>
            <tbody className="divide-y divide-ink/8 dark:divide-white/8">
              {forms.map((f) => (
                <tr key={f.id} className="hover:bg-ink/[0.02] dark:hover:bg-white/[0.02]">
                  <td className="px-4 py-3 min-w-0">
                    <Link href={`/forms/${f.id}/builder`} className="font-medium text-clinical-teal hover:underline block truncate">{f.name}</Link>
              ...
              {forms.length === 0 && (
                <tr>
                  <td colSpan={session.role === "SUPER_ADMIN" ? 6 : 5} className="px-4 py-10 text-center text-sm text-ink-soft/60">
